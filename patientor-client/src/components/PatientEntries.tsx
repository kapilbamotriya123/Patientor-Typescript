...
const PatientEntries: React.FC<PatientEntriesProps> = ({ entry, diagnoseData }) => {
  const getDiagnosisName = (code: string): string => {
    if (!diagnoseData) return 'Unknown diagnosis';
      ...
      <ul className="patient-entry-list">
        ...
        <li className="patient-entry-item">
          ...
          {entry.diagnosisCodes && entry.diagnosisCodes.length > 0 && (
            <div className="patient-entry-diagnoses-list">
              {entry.diagnosisCodes.map((d) => (
                <div key={d} className="patient-entry-diagnosis-item">
